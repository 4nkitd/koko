...
func (fm *FocusManager) ApplyFocus() float32 {
	...
	if vol > 0 {
		fm.origVolume = vol
		// Dip master volume slightly by 25% (e.g. from 80% to 60%) so background audio dips without muffling koko
		duckVolume := vol * 0.75
		if duckVolume < 0.30 {
			duckVolume = 0.30
		}
		...
		if vol > duckVolume {
			fm.gainBoost = 1.35 // 35% extra gain boost for koko speech output
